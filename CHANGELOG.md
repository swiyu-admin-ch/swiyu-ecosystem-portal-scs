...
- Remove trustVerificationStatus as sortable column in business partner list view
...
- Added snyk ignore for CVE-2026-93687
...
- Request additional information flow
...
- Aligned the base onboarding profile creation with the trust onboarding contract: the contact section now captures
  first name, last name and a mandatory correspondence language, and the address is sent as a nested object

## 1.21.21

### Fixed

- Move async navigation in ProtectedVerificationStepConfirmationComponent out of the constructor into ngOnInit to
...
- Fix mapping for submitted submission after passing the profile change flow

## 1.21.18

### Fixed

- Align different states of "Profile verifizieren" tile

## 1.21.17

### Fixed

- Improve trust onboarding wizard route validation to prevent bypassing required steps
...
- Add `spring-boot-security-test` dependency so `@WithJeapAuthenticationToken` works again with MockMvc-based
  integration tests
...
- fixed wrongfully showing pop success message in partner details when partner has no DIDs uploaded
...
- Use correct onboarding / cookbook documentation link
- Set npm project version in setVersion.sh script

## 1.21.9

### Changed

- Add validations on trust onboarding which types of BPs are allowed to onboard
...
- Upgrade to Angular 21 and Oblique 15
...
- Bump angular build version to 20.3.31 to fix CRITICAL vulnerability
...
- update information text for additional document uploade (gov)

## 1.21.1

### Fixed

- Force shell-quote version 1.9.0 to fix HIGH vulnerability
